...
Обучив нейронную сеть с помощью roboflow, мы создали 2 кода. Один из них для квадрокоптера, который фотографирует дорожное полотно и отправляет фотографии на компьютер. И,соответственно, код на компьютер,который обрабатывает пересланные с дрона фотографии,выдает координаты местонахождения ям и присылает телеграмм боту,котрый составляет карту.
